...
void AplicacionIG::inicializarGLFW()
{
   ...
   ventana_glfw = glfwCreateWindow(wx, wy, "Practicas IG GIM+GIADE (24-25)", nullptr, nullptr);
   assert(ventana_glfw != nullptr);

   // Posiciona la ventana
   glfwSetWindowPos(ventana_glfw, px, py);
...
void AplicacionIG::buclePrincipalGLFW()
{
   ...
   while (!terminar_programa)
   {
      ...
      Objeto3D *objeto = escenas[ind_escena_act]->objetoActual();
