...
{
   const float
       sa = std::sin(esfericas[0]),
       ca = std::cos(esfericas[0]),
...
Camara3Modos::Camara3Modos()
{
   ...
   // CUA: añado esto (29 Sept): está en el otro constructor, pero no en este:
   actualizarEjesMCV();
   matrices_actualizadas = false;
}

// Crea una cámara, inicialmente en modo examinar, con el punto de atención en el origen
Camara3Modos::Camara3Modos(const bool perspectiva_ini, const glm::vec3 &origen_ini, const float ratio_vp_ini, const glm::vec3 &punto_aten_ini, const float fovy_grad_ini)
{
   // using namespace std ;
   ...
   const float d_sq = length2(punto_aten_ini - origen_ini);
   assert(0.0 < d_sq);
   ...
   org_cartesianas = origen_ini - punto_aten_ini;
   assert(0.01 <= length(org_cartesianas));
...
void Camara3Modos::desplRotarXY(const float da, const float db)
{
   ...
   case ModoCam::prim_pers_rot:
   {
      ...
      // Calcular el vector de desplazamiento desde las nuevas a las antiguas
      glm::vec3 desplazamiento = (org_cartesianas - aux);

      // Restarle al punto de atención ese vector de desplazamiento
...
void Camara3Modos::actualizarMatrices()
{
   ...
   constexpr float near = 0.05, far = near + 1000.0;
