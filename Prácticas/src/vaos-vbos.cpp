...
{
   assert(mode == GL_TRIANGLES || mode == GL_LINES || mode == GL_POINTS || mode == GL_LINE_STRIP || mode == GL_LINE_LOOP);
}
...
DescrVBOAtribs::DescrVBOAtribs(const unsigned p_index, const GLenum p_type, const unsigned p_size, const unsigned long p_count, const void *p_data)
{
...
void DescrVBOAtribs::crearVBO()
{
   ...
   // Generar un nuevo identificador o nombre de VBO
   glGenBuffers(1, &buffer);
   // Fijar este buffer como buffer 'activo' actualmente en el 'target' GL_ARRAY_BUFFER
   ...
   // Registrar, para este índice de atributo, la localización y el formato de la tabla en el buffer
   glVertexAttribPointer(index, size, type, GL_FALSE, stride, offset);
   // Desactivar el buffer
   glBindBuffer(GL_ARRAY_BUFFER, 0);
   // Habilitar el uso de esta tabla de atributos
...
{
   // Comprobar y limpiar errores previos de OpenGL
   CError();
   // Impedir que se llame más de una vez para este objeto  
   assert(buffer == 0); 
   // Comprobar que este objeto está en un estado correcto
   comprobar();         

   // Crear un nuevo identificador de VBO
   glGenBuffers(1, &buffer);
   // Activar el buffer en el 'target' GL_ELEMENT_ARRAY_BUFFER
...
{
   CError();
   // Asegurarnos que únicamente se invoca una vez para este descriptor
   assert(array == 0); 

   // Crear el identificador de VAO
   glGenVertexArrays(1, &array);
   // Activamos el VAO como VAO actual
   glBindVertexArray(array);
   // Para cada VBO de atributos adjunto al VAO, crear el VBO de atributos en la GPU
   for (unsigned int i = 0; i < dvbo_atributo.size(); i++)
   {
      if(dvbo_atributo[i] != nullptr)
         dvbo_atributo[i]->crearVBO();
   }
   // Si hay índices, crear el VBO de índices
   if (dvbo_indices != nullptr)
   {
      dvbo_indices->crearVBO();
   }
   // Para cada VBO de atributos adjunto al VAO 
   for (unsigned int i = 0; i < dvbo_atributo.size(); i++)
   {
      if (dvbo_atributo[i] != nullptr)
      {
         // Si la tabla está marcada como deshabilitada
         if (atrib_habilitado[i] == false)
            // Deshabilitarla en la GPU
...
void DescrVAO::draw(const GLenum mode)
{
   ...
   CError();
   // Asegurarnos que hay una tabla de coordenadas de posición
   assert(dvbo_atributo[0] != nullptr);
   // Comprobar que el modo es el correcto.
   check_mode(mode);

   // Comprobar si el array se ha creado o no
   if (array > 0)
      // Activar VAO
      glBindVertexArray(array);
   else
      // Crear VAO
      crearVAO();
   // Comprobar si la secuencia es indexada o no lo es
   if (dvbo_indices != nullptr)
